...
public class JobsResource {
	...
	public Response getJobs() {		
		try 
		{	
		List<Job> jobs = _service.getAll(Job.class);
		ArrayList<Object> res = new ArrayList<Object>();
		Iterator<Job> itr = jobs.iterator();
		while(itr.hasNext()) {
			Job job = itr.next();
			res.add(new Object() {public int id=job.getId(); public String name = job.getName() + " level " + job.getLevel();});
		}
	...
	@Path("/{Id}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getCompetencies(@PathParam("Id") int Id) {	
		try {
		Job job = _service.get(Id, Job.class);
		ArrayList<Object> res = new ArrayList<Object>();
		Iterator<Competency> itr = job.getCompetencies().iterator();
		while(itr.hasNext()) {
			Competency comp = itr.next();
			res.add(new Object() {public int id=comp.getId(); public String name = comp.getName(); public int level = comp.getLevel();});
		}
		return Response.status(200).entity(res).build();
		} catch(Exception e) {
			return Response.status(500).entity("An error has occurred. Please verify the data!").build();
